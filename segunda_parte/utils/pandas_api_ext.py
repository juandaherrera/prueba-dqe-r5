...
class MissingMethods:
    """
    Esta clase fue tomada del Curso de Manejo de Datos Faltantes: Detección y Exploración
    de Platzi. url: https://platzi.com/new-home/clases/4196-datos-faltantes
    """
    ...
    def missing_case_summary(self) -> pd.DataFrame:
        return self._obj.assign(
            ...
            n_missing=lambda df: df.apply(axis="columns", func=lambda row: row.isna().sum()),
    ...
    def missing_variable_span(self, variable: str, span_every: int) -> pd.DataFrame:
        return (
            self._obj.assign(
                span_counter=lambda df: (np.repeat(a=range(df.shape[0]), repeats=span_every)[: df.shape[0]])
            )
    ...
    def missing_variable_run(self, variable) -> pd.DataFrame:
        rle_list = self._obj[variable].pipe(lambda s: [[len(list(g)), k] for k, g in itertools.groupby(s.isnull())])
        ...
        return pd.concat(
            objs=[
                ...
                self._obj.missing.create_shadow_matrix(
                    true_string=true_string, false_string=false_string, only_missing=only_missing
                ),
            ],
            axis="columns",
        )
